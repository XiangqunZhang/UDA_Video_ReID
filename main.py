...
parser.add_argument('--save_dir', '--sd', type=str, default='log')
...
def train_c(epoch, model, criterions, optimizer, trainloader, use_gpu):
    ...
    for batch_idx, (vids, pids, camid,_) in enumerate(trainloader):
...
def train1(epoch, model, criterions, optimizer, trainloader, use_gpu):
    batch_xent_loss = AverageMeter()
    batch_htri_loss = AverageMeter()
    batch_info_loss = AverageMeter()
    batch_loss = AverageMeter()
...
def _feats_of_loader1(model, loader, feat_func=extract_feat_sampled_frames, use_gpu=True):
...
def test_mars(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    since = time.time()
    model.eval()
...
def test1(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    ...
    qf, q_pids, q_camids, q_clothes_ids = _feats_of_loader1(
        model,
        queryloader,
        feat_func,
        use_gpu=use_gpu)
    ...
    gf, g_pids, g_camids, g_clothes_ids = _feats_of_loader1(
        model,
        galleryloader,
        feat_func,
        use_gpu=use_gpu)
