...
class Preprocessor_videoDataset1(data.Dataset):
...
class VideoDataset1_vis(data.Dataset):
    """Video Person ReID Dataset.
    Note:
        Batch data has shape N x C x T x H x W
    Args:
    ...
    def __getitem__(self, index):
        ...
        # print(img_paths)
        clip_ids = [path.split('/')[-1].split('_')[2][:5] for path in img_paths]
        # print(clip_ids) #['00734', '00738', '00763', '00702']
        ...
        return clip, pid, camid, clip_ids
...
class ImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # ('/HDD3/zxq/Duke/DukeMTMC-reID/bounding_box_train/2016_c4_f0184829.jpg', 694, 3)
        ...
        if self.spatial_transform is not None:
            self.spatial_transform.randomize_parameters()
            clip = self.spatial_transform(img_paths)
            # clip = [self.spatial_transform(img) for img in clip]
...
class VideoDatasetInfer(data.Dataset):
    ...
    def __getitem__(self, index):
        img_paths, pid, camid, clothes_ids = self.dataset[index]
        ...
        return clip, pid, camid,clothes_ids

class VideoDatasetInfer1(data.Dataset):
    """Video Person ReID Dataset.
    Note:
        Batch data has shape N x C x T x H x W
    Args:
...
class VideoDatasetInfer2(data.Dataset):
    """Video Person ReID Dataset.
    Note:
        Batch data has shape N x C x T x H x W
    Args:
